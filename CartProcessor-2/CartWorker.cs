...
namespace CartWorker
{
    public class AkkaService : IHostedService
    {
        ...
        public AkkaService(ILogger<AkkaService> logger, IServiceProvider provider, IHostApplicationLifetime lifetime)
        {
            _logger = logger;
            _provider = provider;
            _lifetime = lifetime;
        ...
        public  Task StartAsync(CancellationToken cancellationToken)
        {
            ...
            Cluster.Get(_actorSystem).RegisterOnMemberUp(() =>
            {
                ...
                var shardRegion = sharding.Start("cartWorker",Props.Create(() => new CartProcessActor()), ClusterShardingSettings.Create(_actorSystem),
